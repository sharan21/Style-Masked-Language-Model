...
def word_drop_attribution(vocab, x, p, attr):     # drop words with prob based on attribution scores
    x_ = []
    for i in range(x.size(1)):
        words = x[:, i].tolist()
        keep = np.random.rand(len(words)) < attr
        keep[0] = True  # do not drop the start sentence symbol
        sent = [w for j, w in enumerate(words) if keep[j]]
        sent += [vocab.pad] * (len(words)-len(sent))
        x_.append(sent)
...
def embd_noise(embds, args, zeta=0.0):
    ...
    elif(args.noise_type == 'attribution'): #zeta scaled by attribution scores
        noise = (zeta * embeds_magn * args.attr).unsqueeze(-1) * torch.nn.functional.normalize(noise, p=2.0, eps=1e-12, dim=-1) #LBE
    elif(args.noise_type == 'uniform'): #uniform hypersphere in [0, zeta]
